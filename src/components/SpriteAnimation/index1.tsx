...
export const SpriteAnimation: React.FC<SpriteAnimationProps> = ({
  ...
  initialYDesktop = 0, initialYTablet = -250, initialYMobile = -550,
  ...
  useLayoutEffect(() => {
    if (spriteRef.current) {
      ...
      const finalY = isMobile ? 500 : (isTablet ? 1200 : 1450); // Конечное положение по Y
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          frameIndex = Math.floor(scrollProgress * 1.5 * totalFrames) % totalFrames;
